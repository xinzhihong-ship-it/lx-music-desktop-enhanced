...
export default () => {
  ...
  mainHandle<LX.Account.PlatformAccount[]>(
    WIN_MAIN_RENDERER_EVENT_NAME.account_list,
    async() => {
      ...
      for (const account of accounts) {
        ...
        if (!session) continue
        const avatar = await kgProvider
          .getAccountAvatar(session)
          .catch(() => '')
  ...
  mainHandle<string>(
    WIN_MAIN_RENDERER_EVENT_NAME.account_remove,
    async({ params: id }) => {
      sessions.removeAccount(id)
    },
  )

  mainHandle<LX.Account.LoginRequest, LX.Account.PlatformAccount>(
    WIN_MAIN_RENDERER_EVENT_NAME.account_login,
    async({ params }) => {
      if (params.method !== 'cookie' || !params.cookie) { throw new Error('当前登录方式不可用') }
      const result = await providers[params.source].loginByCookie(
        params.cookie,
      )
  ...
  mainHandle<
  { source: LX.Account.Source, requestId: string },
  LX.Account.QrCodeLoginResult
  >(WIN_MAIN_RENDERER_EVENT_NAME.account_qr_check, async({ params }) => {
    const result = await providers[params.source].checkQrCodeStatus(
      params.requestId,
    )
  ...
  mainHandle<string, LX.Account.PlaylistInfo[]>(
    WIN_MAIN_RENDERER_EVENT_NAME.account_playlists,
  ...
  mainHandle<
  { accountId: string, playlistId: string, dirId?: string },
  LX.Account.PlaylistTrackInfo[]
  >(
    WIN_MAIN_RENDERER_EVENT_NAME.account_playlist_tracks,
    async({ params }) => {
      const session = sessions.getSession(params.accountId)
      if (!session) throw new Error('账号登录状态不存在或已失效')
      return providers[session.source].getPlaylistTrackIds(
        session,
        params.playlistId,
        params.dirId,
      )
  ...
  mainHandle<LX.Account.PlaylistMutationRequest>(
    WIN_MAIN_RENDERER_EVENT_NAME.account_playlist_add_tracks,
    async({ params }) => {
      const session = sessions.getSession(params.accountId)
      if (!session) throw new Error('账号登录状态不存在或已失效')
      if (params.tracks.some((track) => track.source !== session.source)) { throw new Error('歌曲来源与目标平台不一致') }
      await providers[session.source].addPlaylistTracks(
        session,
        params.playlistId,
        params.dirId,
        params.tracks,
  ...
  mainHandle<LX.Account.PlaylistMutationRequest>(
    WIN_MAIN_RENDERER_EVENT_NAME.account_playlist_remove_tracks,
    async({ params }) => {
      const session = sessions.getSession(params.accountId)
      if (!session) throw new Error('账号登录状态不存在或已失效')
      if (params.tracks.some((track) => track.source !== session.source)) { throw new Error('歌曲来源与目标平台不一致') }
      await providers[session.source].removePlaylistTracks(
        session,
        params.playlistId,
        params.dirId,
        params.tracks,
  ...
  mainHandle<string, string[]>(
    WIN_MAIN_RENDERER_EVENT_NAME.account_daily_tracks,
    async({ params: accountId }) => {
      const session = sessions.getSession(accountId)
      if (!session) throw new Error('账号登录状态不存在或已失效')
  ...
  mainHandle<LX.Music.SimilarSongsRequest, LX.Music.SimilarSongsResult>(
    WIN_MAIN_RENDERER_EVENT_NAME.music_similar_tracks,
    async({ params }) => {
      ...
      switch (params.source) {
        case 'wy':
          list = await wyProvider.getSimilarSongs(
            sessions.getSessionBySource('wy'),
            params.songId,
            limit,
          )
          break
        case 'tx':
          list = await txProvider.getSimilarSongs(
            sessions.getSessionBySource('tx'),
          ...
          break
        case 'kg':
          list = await kgProvider.getSimilarSongs(
            sessions.getSessionBySource('kg'),
            params.hash ?? '',
            String(params.songId),
            limit,
          )
          break
        case 'bili':
          list = await biliProvider.getSimilarSongs(
            sessions.getSessionBySource('bili'),
            String(params.songId),
            limit,
          )
          break
  ...
  mainHandle<LX.Account.Source, string>(
    WIN_MAIN_RENDERER_EVENT_NAME.account_source_cookie,
    async({ params: source }) => {
