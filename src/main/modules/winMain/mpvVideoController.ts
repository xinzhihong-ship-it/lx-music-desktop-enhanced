...
const ensureExternalVideoPlayer = () => {
	if (processPlayer && videoHostWindow && !videoHostWindow.isDestroyed())
		...
		for (const event of native.poll()) {
			switch (event.name) {
				...
				case "loaded":
					...
					try {
						if (native.getProperty("pause") === false)
							sendVideoEvent("playing");
					} catch (error) {
				...
				case "time":
					if (typeof event.value === "number")
						sendVideoEvent("timeUpdate", event.value);
					break;
				case "duration":
					if (typeof event.value === "number")
						sendVideoEvent("duration", event.value);
					break;
				case "pause":
					if (loading) break;
...
export const loadUrl = async (videoUrl: string, audioUrl?: string) => {
	if (!useNativeMacVideo) {
		...
		try {
			...
			await videoHostReady;
			if (!videoHostWindow || videoHostWindow.isDestroyed())
